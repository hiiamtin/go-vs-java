...
# Test script for Spring Boot application with database
# Tests all endpoints including database-dependent ones

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$SCRIPT_DIR" || exit 1

echo "🧪 Testing Spring Boot Application with Database"
echo "==============================================="
...
if [ "$http_code" = "201" ]; then
    ...
    if echo "$response_body" | grep -q '"customer_id":10'; then
        ...
        echo "   Interaction ID: $(echo "$response_body" | grep -o '"id":[0-9]*' | cut -d: -f2)"
...
echo ""
echo "🚀 Spring Boot application is fully functional and ready for POC!"
